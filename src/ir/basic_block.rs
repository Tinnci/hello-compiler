...
impl BasicBlock {
    ...
    pub fn add_instruction(&mut self, instruction: InstructionRef) {
        // 设置指令的父基本块
        instruction.borrow_mut().set_parent(Some(Rc::new(RefCell::new(self.clone()))));
    ...
    pub fn remove_instruction(&mut self, instruction: &InstructionRef) -> bool {
        if let Some(pos) = self.instructions.iter().position(|i| Rc::ptr_eq(i, instruction)) {
            // 清除指令的父基本块
...
impl fmt::Debug for BasicBlock {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "BasicBlock({}, {} instructions)", self.get_name(), self.instructions.len())
    }
} 